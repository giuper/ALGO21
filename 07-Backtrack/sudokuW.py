...
class Sudoku(BackTrack):
    ...
    def Solve(self,verbose=False):
        res=self._Solve([self.P,0,0],verbose)
        ##prima componente True/False
        if res[0]:
            print("Soluzione: ")
            #seconda componente stato finale
            #prima componente dello stato finale
            #tavola totalmente riempita
            for row in res[1][0]:
                print(row)
            print()
        else:
